...
for i = 1:max(DB.Z(:))
    ...
    arr(i,1) = i; % Valley ID number
    arr(i,2) = xsnap(i); % Outlet Easting
    arr(i,3) = ysnap(i); % Outlet Northing
    arr(i,4) = cal_dist; % D_min (distance to volcanic center)
    arr(i,5) = rain_fac; % MAP (mean annual precipitation)
    arr(i,6) = Abkm2; % Drainage area
    arr(i,7) = rel_fac; % Mean relief (depending on settings above)
    arr(i,8) = slp_fac; % Mean slope
    arr(i,9) = wid_fac; % Longest valley width
    arr(i,10) = len_fac; % Valley length
    arr(i,11) = asp_ratio; % Valley aspect ratio as width/length
    arr(i,12) = vol_eroded_surf1; % Volume eroded
    arr(i,13) = vol_err_frac; % Volume gained in error from reconstructed surface
    arr(i,14) = fx; % Fraction eroded (following Ferrier et al., 2013 GSA Bulletin)
    arr(i,15) = Ev; % Volumetric erosion rate
    arr(i,16) = Ev_1s; % Volumetric erosion rate standard deviation
    arr(i,17) = Ev_v; % Vertical erosion rate
    arr(i,18) = Ev_v_1s; % Vertical erosion rate standard deviation
    arr(i,19) = BA_ksn; % ksn from basin averaged values (e.g., Wobus et al., 2006)
    arr(i,20) = BA_ksn_1s; % ksn from basin averaged values standard deviation
    arr(i,21) = C.beta; % ksn from the integral method
    arr(i,22) = Cbeta_1s; % ksn from the integral method standard deviation
    arr(i,23) = K_BA*1e-0; % K from basin averaged values
    arr(i,24) = K_BA_1s*1e-0; % K from basin averaged values standard deviation
    arr(i,25) = K_chi*1e-0; % K from integral method
    arr(i,26) = K_chi_1s*1e-0; % K from integral method standard deviation
    arr(i,27) = BA_ksnq; % ksn-q from basin averaged values
    arr(i,28) = Kq_BA*1e-0; % Klp from basin averaged values
    arr(i,29) = Kq_BA_1s*1e-0; % Klp from basin averaged values standard deviation
    arr(i,30) = CQ.beta; % ksn-q from integral method
    arr(i,31) = Klp_chi*1e-0; % Klp from integral method
    arr(i,32) = Klp_chi_1s*1e-0; % Klp from integral method standard deviation
    arr(i,33) = Klp_MAP*1e-0; % Klp from K/MAP^m (e.g., Adams et al., 2020)
    arr(i,34) = Klp_MAP_1s*1e-0; % Klp from K/MAP^m standard deviation
    arr(i,35) = s0_fac; % 95th steepest percentile of relict slope (depending on settings above)
