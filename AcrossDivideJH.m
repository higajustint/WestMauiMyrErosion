function [head_vals,out_str]=AcrossDivideJH(DEM,FD,DS_OUT,varargin)
    ...
    %       out_str - 1x4 cell with labels of drainage migration for 1)elevation, 2) relief, 3) gradient, 4) chi %% JH 4/6/2024
    %
    % Examples:
    ...
    addParamValue(p,'divide_buffer','moderate',@(x) ischar(validatestring(x,{'conservative','moderate','lax','ultralax'}))); % ultralax JH 4/6/2024
    ...
    if strcmp(outlet_method,'picked_outlets')
        ...
        for jj=1:num_divides
            ...
            case 'ultralax' % JH 4/6/2024
                cut_dist=3.5*sqrt(ra); % JH 4/6/2024
            end
        ...
        case 'lax'
            cut_dist=3.0*sqrt(ra);
        case 'ultralax' %%% JH 4/5/2024
            cut_dist=3.5*sqrt(ra); %%% JH 4/5/2024
        end
